# Roadmap B – Design, Interface und Lesbarkeit

Status: **vorbereitet, noch nicht aktiv**. Roadmap B startet erst nach der Abnahme von Block A.08.

Die gemessene Ausgangsbasis mit 16 Flächen, Zustandsmatrix und priorisiertem UX-Backlog steht in `ui/SCENE_INVENTORY.md`. Dieser Befund ist verbindlicher Eingang für B.01 und wird nicht während der Umsetzung stillschweigend umgedeutet.

## Ziel

Roadmap B macht aus dem technisch funktionierenden Systemfundament ein geschlossenes, modernes und verständliches Spiel. Sie verändert Darstellung, Navigation und Bedienung, aber keine serverautoritativen Wirtschaftsregeln. Neue große Spielsysteme gehören weiterhin in Roadmap C.

## Acht geplante Designblöcke

| Block | Schwerpunkt | Zielbild |
| --- | --- | --- |
| **B.01** | Bestandsaufnahme und Designsystem | vollständiges UI-Inventar, Tokens, Raster, Typografie und Komponentenvertrag |
| **B.02** | Informationsarchitektur und Navigation | klare Wege zwischen Kampf, Sammlung, Brut, Forschung, Gilde und Profil |
| **B.03** | Kampfszene und HUD | maximaler Kampffokus, einklappbare Nebeninformationen und saubere Prioritäten |
| **B.04** | Sammlung und Fortschritt | verständliche Monster-, Gem-, Ei-, Forschungs- und Prestigeansichten |
| **B.05** | Tamer-Identität | runde Profilbilder, Avatar-Katalog, wechselbare Rahmen und Profilpräsentation |
| **B.06** | Gilden- und Sozialoberfläche | lesbare DNA, Mitglieder, Boss, Aufgaben, Chat, Freunde und Moderationsfeedback |
| **B.07** | Responsive Design und Zugänglichkeit | Desktop, Tablet, Mobile, Kontrast, Skalierung, Fokusführung und reduzierte Bewegung |
| **B.08** | Gesamtpolish und Übergabe | konsistente Animationen, Feedback, Screenshots, Designabnahme und Übergabe an C |

Jeder Block durchläuft erneut die vier Gates **Planen → Bauen → Prüfen → Abnehmen**.

## Verbindliche Leitplanken

- Silberne Akzente und leichte violette Elemente bleiben die visuelle Basis.
- Die Kampfszene bleibt die Hauptszene und darf nicht wieder zu einem kleinen Dashboard-Baustein werden.
- Häufige Aktionen liegen direkt in der jeweiligen Szene; seltene Details dürfen in Panels und Dialoge wandern.
- Schriftgröße, Kontrast und Informationsdichte werden an realen Viewports geprüft.
- Avatare und Rahmen sind getrennte, serverseitig gespeicherte Katalogeinträge.
- Monster- und Itemgrafiken bleiben HD; PixelLab-Animationen verwenden den 200×200-Vertrag.
- UI-Arbeit darf keine Gold-, Besitz-, Zeit- oder Gildenautorität zurück in den Browser verlagern.
- Roadmap B fügt keine großen Content- oder Wettbewerbssysteme hinzu.

## Abnahmeziel

Am Ende von Roadmap B besitzt jede relevante Funktion einen konsistenten Desktop- und Mobile-Zustand inklusive Laden, Leerzustand, Fehler, Sperre, Erfolg und Bestätigung. Die Oberfläche ist dann bereit, in Roadmap C mit umfangreicherem Content und neuen Features gefüllt zu werden.
